...
QList<int> CyclesSyntax::Prog()
{
	...
	int uk1 = scaner->GetUk();		// переменна€ дл€ хранени€ указател€ в тексте программы
	int type_lex = scaner->Scan(lex);	// переменна€, хран€ща€ текущий тип данных, считанный лексером
	...
	while (type_lex != TYPE_END && type_lex != TYPE_CLOSED_BRACE)
	{
		...
		{
			//std::cout << "ƒошли до конца модул€!" << std::endl;
...
void CyclesSyntax::E()
{
	...
	// запоминаем текущий узел дерева (дл€ возврата из вложенности)
	BaseMetricsTree* save_parent = this->currentNode;
	std::string name = ""; // им€ цикла дл€ отладки
	...
	if (type_lex == TYPE_FOR)
	{
		...
		type_lex = scaner->Scan(lex);
		// после for открываетс€ скобка, далее не важно что... 
		if (*lex == "(")
		{
			while (*lex != ")")
			{
				...
				{
                    //std::cout << "ќжидалась скобка ')' дл€ оператора цикла for";
                    //exit(-1);
                    QMessageBox* pmbx =
                                        new QMessageBox(" ритическа€ ошибка!",
                                        "ќжидалась скобка ')' дл€ оператора цикла for",
			...
			// тут можно ещЄ отслеживать однострочные циклы...
			if (type_lex == TYPE_OPENED_BRACE)
			{
                ...
                //std::cout << "\nѕереходим в " << name << " (" << this->currentNode << ")";
				// вложенность
	...
	else if (type_lex == TYPE_DO)
	{
		...
		if (type_lex != TYPE_WHILE)
		{
            QMessageBox* pmbx =
                                new QMessageBox(" ритическа€ ошибка!",
                                "ожидаетс€ оператор while дл€ do-while.",
                                QMessageBox::Critical,
                                QMessageBox::Ok,
                                0,
                                0);
            pmbx->exec();
            delete pmbx;
            //std::cout << "ожидаетс€ оператор while дл€ do-while.";
            exit(-1);
		}
    ...
    //std::cout << "\n¬ыходим из " << name << " возврат в (" << save_parent << ")";
